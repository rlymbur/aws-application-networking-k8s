...
func TestRouteReconciler_WithoutPriority_ReconcileCreates(t *testing.T) {
	config.VpcID = "my-vpc"
	...
	// Register DNS endpoint types only since anv1alpha1 types are already registered
	dnsEndpoint := schema.GroupVersion{
		Group:   "externaldns.k8s.io",
		Version: "v1alpha1",
	}
	k8sScheme.AddKnownTypes(dnsEndpoint, &endpoint.DNSEndpoint{}, &endpoint.DNSEndpointList{})
	metav1.AddToGroupVersion(k8sScheme, dnsEndpoint)

	k8sClient := testclient.
		NewClientBuilder().
		WithScheme(k8sScheme).
		WithStatusSubresource(&anv1alpha1.HTTPRoute{}).
		Build()
...
func TestRouteReconciler_WithPriority_ReconcileCreates(t *testing.T) {
	config.VpcID = "my-vpc"
	config.ClusterName = "my-cluster"
	...
	kind := gwv1.Kind("Service")
	port := gwv1.PortNumber(80)
	routeWithPriority := anv1alpha1.HTTPRoute{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "my-route-with-priority",
			Namespace: "ns1",
		},
	...
	k8sClient.Create(ctx, routeWithPriority.DeepCopy())
	...
	// Reconcile the route with priority
	priorityRouteName := k8s.NamespacedName(&routeWithPriority)
	result, err := rc.Reconcile(ctx, reconcile.Request{NamespacedName: priorityRouteName})
	assert.Nil(t, err)
	assert.False(t, result.Requeue)
}
